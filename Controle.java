...
public class Controle {
    ...
    private void entityColision(){
        for (int i = 0; i < al.size(); i++){
            Ammo ammo = al.get(i);
            int difX = personagem.getX() - ammo.getX();
            int difY = personagem.getY() - ammo.getY();
    ...
    private void zombieColision(){
        for(int i=0;i<zl.size();i++){
            zombie = zl.get(i);
            int difX = zombie.getX()-personagem.getX();
            int difY = zombie.getY()-personagem.getY();
            double distance = Math.sqrt(difX*difX+difY*difY);
            /*if(zombie.distance <= 30){
                zombie.attack();
            }*/
